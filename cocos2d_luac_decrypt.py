...
def extract_apk(apkPath, outputDir="output", sign=None, key=None):
    ...
    if os.path.exists(apkPath):
        ...
        for names in zipFile.namelist():
            ...
            if os.path.splitext(names)[-1] == '.luac' or os.path.splitext(names)[-1] == '.lua':
                if sign and key:
...
def decrypt_dir(dir, sign, key):
    for root, dirs, files in os.walk(dir):
        for f in files:
            if os.path.splitext(f)[-1] == ".luac" or os.path.splitext(f)[-1] == ".lua":
                xxtea.decrypt_file(src_file=os.path.join(
...
def get_guess_key_list(may_contain):
    may_contain = re.sub(r"[^a-zA-Z0-9 !@#$%\^&\*\(\)_=]", " ", may_contain)
...
def main():
    parser = argparse.ArgumentParser(
        add_help=True, description="cocos2d lua decrypt tool",
